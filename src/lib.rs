...
use ffi_types::{
    ...
    OBS_MEDIA_STATE_PAUSED, OBS_MEDIA_STATE_PLAYING, OBS_MEDIA_STATE_STOPPED, OBS_PATH_DIRECTORY,
    OBS_TEXT_DEFAULT,
};
...
unsafe extern "C" fn start_game_clicked(
    ...
    let mut process = match state.use_game_arguments {
        true => {
        ...
        false => {
            if state.game_path.exists() {
                info!("Starting game...");
                Command::new(state.game_path.clone())
            } else {
                warn!("No path provided to start a game.");
                return false;
            }
